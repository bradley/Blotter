  ...
  Blotter._extendWithGettersSetters(Blotter.GhostBlurMaterial.prototype, (function () {
    ...
    function _mainImageSrc () {
      var mainImageSrc = [
        ...
        "    intersectsOnRectForLine(perpendicularIntersectA, perpendicularIntersectB, vec2(0.0) - amplitude, uResolution.xy + amplitude, coordA, perpendicularSlope); ",
        "    ",
        "    ",
        "    // Prepare wave",
        "    // -------------------------------",
        ...
        "    lowp float dDeg = 0.0;", // 0.0 deg is default.
        "    float dWidth = 0.96 * uResolution.y;", // should be (default) 86% of length of diagnoal vector
        "    float dAmplitude = 0.185 * dWidth;", // should be 20% (default) of dWidth
        "    float dSmoothing = 2.975 * dAmplitude;", // should be 325% (default) of dAmplitude
        "    float dVolatility = 0.55;", // number between 0 and 1. default is .345;
        "    float amount = isWave(uv, dPosition, dDeg, dWidth, dSmoothing, dAmplitude, dVolatility);",


        "    // Create Darkness ==============================================================",

        "    vec2 maxRadiusUv = vec2(0.0337, 0.0955);// + (0.125 * amount));",
        ...
        "            vec4 sampleOnBackground = normalBlend(stepSampleAdjusted, uBlendColor);",
