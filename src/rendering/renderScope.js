import { extend, reduce, bind, each } from "underscore";
...
var RenderScope = function (text, blotter) {
  this.text = text;
  ...
  this.domElement = CanvasUtils.hiDpiCanvas(0, 0, this.blotter.ratio, {
    class : "b-canvas",
    html : text.value
  });
...
RenderScope.prototype = (function () {
  ...
  function _setMouseEventListeners () {
    ...
    function setMouseListener (eventName) {
      self.domElement.addEventListener(eventName, function(e) {
        var position = CanvasUtils.normalizedMousePosition(self.domElement, e);
  ...
  function _transferInferfaceValues (oldInterface, newInterface) {
    each(oldInterface, function(interfaceObject, uniformName) {
  ...
  function _getUniformInterfaceForUniformDescription (uniformDescription) {
    var uniformInterface = {
      ...
      set type (v) {
        Messaging.logError("Blotter.RenderScope", false, "uniform types may not be updated through a text scope");
      },
      ...
      set value (v) {
        if (!UniformUtils.validValueForUniformType(this._type, v)) {
          Messaging.logError("Blotter.RenderScope", false, "uniform value not valid for uniform type: " + this._type);
          return;
        }
        this._value = v;
...
extendWithGettersSetters(RenderScope.prototype, EventEmitter.prototype);
