import { reduce, isArray, difference, allKeys, pick } from "underscore";
...
var UniformUtils = {
  ...
  defaultUniforms : {
    ...
    uPixelRatio : { type : "1f", value : CanvasUtils.pixelRatio } // The pixel ratio of the user's device
  },
  ...
  validValueForUniformType : function (type, value) {
    ...
    switch (type) {
      ...
      case "2f":
        valid = isArray(value) && value.length == 2 && value.every(isValid);
        break;

      case "3f":
        valid = isArray(value) && value.length == 3 && value.every(isValid);
        break;

      case "4f":
        valid = isArray(value) && value.length == 4 && value.every(isValid);
        break;
  ...
  extractValidUniforms : function (uniforms) {
    uniforms = uniforms || {};
    return reduce(uniforms, function (memo, uniformDescription, uniformName) {
      if (UniformUtils.UniformTypes.indexOf(uniformDescription.type) == -1) {
        Messaging.logError("Blotter.UniformUtils", "extractValidUniforms", "uniforms must be one of type: " +
          UniformUtils.UniformTypes.join(", "));
        return memo;
      }

      if (!UniformUtils.validValueForUniformType(uniformDescription.type, uniformDescription.value)) {
        Messaging.logError("Blotter.UniformUtils", "extractValidUniforms", "uniform value for " + uniformName + " is incorrect for type: " + uniformDescription.type);
        return memo;
      }

      memo[uniformName] = pick(uniformDescription, "type", "value");
      return memo;
    }, {});
  },

  ensureHasRequiredDefaultUniforms : function (uniforms, domain, method) {
    if (!(UniformUtils.hasRequiredDefaultUniforms(uniforms))) {
  ...
  hasRequiredDefaultUniforms : function (uniforms) {
    var missingKeys = difference(allKeys(UniformUtils.defaultUniforms), allKeys(uniforms));
