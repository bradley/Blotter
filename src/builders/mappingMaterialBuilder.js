...
import { BoundsDataTextureBuilder } from "./textures/boundsDataTextureBuilder";
import { IndicesDataTextureBuilder } from "./textures/indicesDataTextureBuilder";
import { TextTextureBuilder } from "./textures/textTextureBuilder";


var MappingMaterialBuilder = (function() {
  ...
  function _fragmentSrc (userUniformDataTextureObjects, textsLength, mainImageSrc) {
    ...
    reduce(userUniformDataTextureObjects.userUniforms, function (userUniforms, uniformObject, uniformName) {
      var glslSwizzle = UniformUtils.fullSwizzleStringForUniformType(uniformObject.userUniform.type),
          glslDataType = UniformUtils.glslDataTypeForUniformType(uniformObject.userUniform.type);
  ...
  function _buildMappedTextsTexture (mapping, completion) {
    TextTextureBuilder.build(mapping, function (texture) {
      completion(texture);
    });
  }
  ...
  function _buildMappingDataTextureObjects (mapping, completion) {
    ...
    buildIndicesTexture = function () {
      return function (next) {
        IndicesDataTextureBuilder.build(mapping, function (texture) {
    ...
    buildBoundsTexture = function () {
      return function (next) {
        BoundsDataTextureBuilder.build(mapping, function (texture) {
          mappingDataTextureObjects.push({
  ...
  function _buildUserUniformDataTextureObjects (userUniforms, textsLength, completion) {
    ...
    reduce(userUniforms, function (memo, userUniform, uniformName) {
  ...
  function _getUniformsForMappingDataTextureObjects (mappingDataTextureObjects) {
    return reduce(mappingDataTextureObjects, function (memo, mappingDataTextureObject) {
  ...
  function _getUniforms (width, height, mappedTextsTexture, mappingDataTextureObjects, userUniformDataTextureObjects) {
    ...
    extend(uniforms, _getUniformsForMappingDataTextureObjects(mappingDataTextureObjects));
    extend(uniforms, _getUniformsForUserUniformDataObjects(userUniformDataTextureObjects));
    ...
    build : function (mapping, material, completion) {
      ...
      reduceRight(buildStages, wrap, function () {
        var uniforms = _getUniforms(
            ...
            userUniforms = omit(uniforms, "_uCanvasResolution", "_uSampler", "_uTextBoundsTexture", "_uTextIndicesTexture"),
        ...
        completion(new MappingMaterial(mapping, material, threeMaterial, userUniformDataTextureObjects));
      })();
    }
  };
})();
